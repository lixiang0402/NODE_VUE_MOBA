  ...
  <div>
    ...
    <el-table :data="items">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="$router.push(`/heroes/edit/${scope.row._id}`)">编辑</el-button>
  ...
  methods: {
    ...
    async deleteCate(row) {
      this.$confirm(`是否删除分类 :${row.name}`, '提示', {
        ...
        const { status } = await this.$http.delete(`/rest/heroes/${row._id}`)
